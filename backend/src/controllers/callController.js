...
// ----------------------------------------------------------------------
// 🏠 NEW FUNCTION: Fetch Address Details by Address ID
// ----------------------------------------------------------------------

/**
 * Fetches a single address entry from the 'Address' table using the address_id.
 * This directly supports the /call/address/details/:addressId endpoint used by the frontend.
 */
exports.getAddressDetailsById = async (req, res) => {
    // 1. Get the addressId from the URL parameters (which the frontend is sending)
    const addressId = req.params.addressId; 

    if (!addressId) {
        console.error('🚨 [SINGLE ADDRESS FAIL] Missing addressId in request parameters.');
        return res.status(400).json({ message: 'Missing address ID.' });
    }
    console.log(`[SINGLE ADDRESS START] Initiating query for address_id: ${addressId}`);

    try {
        // --- QUERY: Fetch the address using the address_id ---
        const { data: addresses, error } = await supabase
            .from('Address')
            // Select all fields needed for a complete address display
            .select('address_id, address_line, city, state, zip_code') 
            .eq('address_id', addressId) // Use the addressId from the URL
            .limit(1); 
            
        if (error) {
            console.error("❌ [SINGLE ADDRESS ERROR] Supabase query failed:", error.message);
            return res.status(500).json({ message: 'Database query failed.', details: error.message });
        }
        ...
        if (!address) {
            console.warn(`⚠️ [SINGLE ADDRESS NOT FOUND] Address ID ${addressId} returned zero results.`);
            return res.status(404).json({ message: 'Address not found.' });
        }
        
        console.log(`✅ [SINGLE ADDRESS SUCCESS] Found address for ID ${addressId}.`);

        // Respond with the address object (matching the frontend's expected structure)
        res.status(200).json({
            message: 'Address fetched successfully.',
            address: address // Returns the single address object
        });

    } catch (e) {
        console.error("🛑 [SINGLE ADDRESS EXCEPTION] Internal server exception caught:", e.message);
        res.status(500).json({ message: 'Internal server error during address lookup.' });
    }
};
